...
if __name__ == '__main__':
    ...
    train_loader = paddle.io.DataLoader(
        ...
        places=place, batch_size=8, shuffle=True,
    ...
    # ---------------------------------------------Basic_resnet20----------------------------------------------------
    model = resnet20()
    epochs = 2  # 当train为固定迭代次数时，这里就是迭代次数。若不是固定迭代次数，则此参数无效
    save_every = 1
    ...
    opt = paddle.optimizer.Momentum(parameters=model.parameters(), learning_rate=scheduler, momentum=0.9,
                                    weight_decay=1e-4)
    # opt = paddle.optimizer.Adam(parameters=model.parameters(), learning_rate=scheduler)
    save_dir = 'models/cifar10/resnet20'
    ...
    df1.to_excel(writer, sheet_name='resnet20')
    # writer.save()
    # ---------------------------------------------Basic_resnet20----------------------------------------------------
    ...
    # save_dir = 'models/cifar10/resnet32'
    # mydata = training()
    # df2 = pd.DataFrame(mydata)
    # df2.to_excel(writer,sheet_name='resnet32')
    # ---------------------------------------------Basic_resnet32----------------------------------------------------
    # ---------------------------------------------Basic_resnet44----------------------------------------------------
    # model = resnet44()
    # epochs = 200
    ...
    # scheduler=paddle.optimizer.lr.PiecewiseDecay(boundaries=[80,120],values=[0.1,0.01,0.001],verbose=True)
    # opt = paddle.optimizer.Momentum(parameters=model.parameters(), learning_rate=scheduler, momentum=0.9,weight_decay=1e-4)
    # save_dir = 'models/cifar10/resnet44'
    # mydata = training()
    # df3 = pd.DataFrame(mydata)
    # df3.to_excel(writer,sheet_name='resnet44')
    # ---------------------------------------------Basic_resnet44----------------------------------------------------
    # ---------------------------------------------Basic_resnet56----------------------------------------------------
    # model = resnet56()
    # epochs = 200
    ...
    # scheduler=paddle.optimizer.lr.PiecewiseDecay(boundaries=[80,120],values=[0.1,0.01,0.001],verbose=True)
    # opt = paddle.optimizer.Momentum(parameters=model.parameters(), learning_rate=scheduler, momentum=0.9,weight_decay=1e-4)
    # save_dir = 'models/cifar10/resnet56'
    # mydata = training()
    # df3 = pd.DataFrame(mydata)
    # df3.to_excel(writer,sheet_name='resnet56')
    # ---------------------------------------------Basic_resnet56----------------------------------------------------
    # ---------------------------------------------Basic_resnet110----------------------------------------------------
    # model = resnet110()
    # epochs = 200
    ...
    # scheduler=paddle.optimizer.lr.PiecewiseDecay(boundaries=[80,120],values=[0.1,0.01,0.001],verbose=True)
    # opt = paddle.optimizer.Momentum(parameters=model.parameters(), learning_rate=scheduler, momentum=0.9,weight_decay=1e-4)
    # save_dir = 'models/cifar10/resnet110'
    # mydata = training()
    # df3 = pd.DataFrame(mydata)
    # df3.to_excel(writer,sheet_name='resnet110')
    # ---------------------------------------------Basic_resnet110----------------------------------------------------
    writer.save()
# ---------------------------------------------save---------------------------------------------------------------
