...
class ConvNet(object):
    ...
    def SGD(self, train_image, train_label, test_image, test_label, epochs, mini_batch_size, eta):
        ...
        for j in range(epochs):
            ...
            if j >= 1:
                #计算前后两次损失值的落差
                height = abs(fy_loss[j-1] - loss)
                #判断落差是否小于等于数值ξ
                if height <= abs(loss-sum(fy_loss)/len(fy_loss))*math.exp(-(1e-5)*j):
                    #将height放入loss_height数组中
                    ...
                    flag = False
                    loss_height.clear()
            print(" After epoch{0}: accuracy is {1}/{2},loss is {3},{4}".format(j + 1, accurate_num, len(test_image), loss,flag))
...
def main():
    ...
    net.SGD(train_image, train_label, test_image, test_label, 100, 100, 1e-5)
