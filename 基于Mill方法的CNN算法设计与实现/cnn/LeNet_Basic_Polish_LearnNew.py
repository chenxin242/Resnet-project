...
def dataset_loader():
    train_image = read_image(r'./mnist_data/train-images.idx3-ubyte')
    train_label = read_label(r'./mnist_data/train-labels.idx1-ubyte')
    test_image = read_image(r'./mnist_data/t10k-images.idx3-ubyte')
    test_label = read_label(r'./mnist_data/t10k-labels.idx1-ubyte')
...
class ConvNet(object):
    ...
    def SGD(self, train_image, train_label, test_image, test_label, mini_batch_size, eta):
        '''
        随机梯度下降法，需要送入训练数据，训练标签，测试数据，测试标签，batch_size大小，学习率
        '''
        batch_num = 0
        decay_rate = 0.9
        decay_epoch = 10
        ...
        while True:
            ...
            for mini_batch_image, mini_batch_label in zip(mini_batches_image, mini_batches_label):
                ...
                print("\rEpoch{0}:{1}/{2}".format(epoch, batch_num * mini_batch_size, len(train_image)), end='')
            accurate_num, loss = self.evaluate(test_image, test_label)
            plt.figure(1)
            fx.append(epoch)
            fy_accuracy.append((0.0 + accurate_num) / len(test_image))
            fy_loss.append(loss / len(test_image))
            # fy_loss.append(loss / len(test_image)) 代替 fy_loss.append(loss)
            loss = loss / len(test_image)
            if epoch >= 1:
                # 学习率的更新
                eta = eta * (math.exp(-decay_rate/decay_epoch))
                height = abs(fy_loss[epoch-1] - loss)
                if height <= abs(loss-sum(fy_loss)/len(fy_loss))*math.exp(-(eta)*epoch):
                    loss_height.append(height)
                    flag = True
                    if len(loss_height) >= int(1/200*accurate_num):
                        break
                else:
                    flag = False
                    loss_height.clear()
            print(" After epoch{0}: accuracy is {1}/{2},loss is {3},{4}".format(epoch, accurate_num, len(test_image), loss,flag))
            print("当前学习率：{0}".format(eta))
            epoch += 1
...
def main():
    ...
    # 初试化卷积神经网络
    net = ConvNet()
    # 对卷积神经网络进行训练，设定好训练数据，验证数据，batch大小和学习率
    net.SGD(train_image, train_label, test_image, test_label, 100, 3e-5)
    # 1e-5  换成了3e-5 学习率 正则化
